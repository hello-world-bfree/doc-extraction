...
class EpubParser:
    ...
    def load(self):
        ...
        self.provenance = {
            "doc_id": stable_id(
                os.path.abspath(self.epub_path), str(os.path.getmtime(self.epub_path))
            ),
    ...
    def parse(self):
        ...
        LOGGER.info(
            "✓ Extracted %d paragraphs; route=%s (%.2f)",
            len(self.chunks),
            self.doc_route,
    ...
    def _write_hierarchy_report(self, filename: str):
        ...
        for path, para_ids in structures.items():
            ...
            word_count = sum(
                ch["word_count"] for ch in self.chunks if ch["paragraph_id"] in para_ids
            )
...
def main():
    ...
    def process_one(epub_path: str):
        ...
        try:
            ...
            print(
                f"   • paragraphs: {len(parser_obj.chunks)}   quality: {round(parser_obj.doc_quality_score, 3)} (route {parser_obj.doc_route})"
            )
            print(
    ...
    if not epubs:
        LOGGER.error(
            "No .epub files found in the provided directory%s.",
